...
def verificar_nota_existente(nota_fiscal):
    ...
    if conn:
        ...
        try:
            cursor.execute(
                """
                SELECT COUNT(*) FROM notafiscaiscanhotoalt
                WHERE NumeroNota = %s
                """,
                (nota_fiscal,)
            )
...
def salvar_imagem_no_banco(imagem, nota_fiscal):
    ...
    if conn:
        ...
        try:
            ...
            cursor.execute(
                """
                INSERT INTO notafiscaiscanhotoalt (NumeroNota, DataBipe, CaminhoImagem, Imagem)
                VALUES (%s, %s, %s, %s)
                """,
    ...
    if conn:  # Verifica se a conexão foi bem-sucedida
        try:
            with conn.cursor() as cursor:
                ...
                cursor.execute("SELECT COUNT(*) FROM notafiscaiscanhotoalt")
...
<div class="footer">
    Desenvolvido.: Panavarro | <a href="mailto:thiago.panuto@dinatec.com.br">Suporte</a>
</div>
"""
...
with st.sidebar:
    ...
    st.markdown(
        ...
        f"🏭 Ribeirão Preto<br>Qtd. Canhotos:<br>🔗{quantidade_canhotos}</h3>", unsafe_allow_html=True)
...
# Consultar nota fiscal no SQL Server
    if st.button("Consultar Nota Fiscal no SQL Server"):
        if nota_fiscal:
